...
export const createRoutes = () => {
	...
	router.post("/kpi-records/submit", authRequired(), validate(kpiSubmitSchema), asyncHandler(async (req, res) => {
		const body = { ...req.body };
		const targetCandidates = await appRepository.listKpiTargets({ userId: req.user.id, periodId: body.periodId });
		const target = targetCandidates.find((item) => Number(item.userId) === Number(req.user.id)) || null;
	...
	router.get("/reports/export", authRequired(), asyncHandler(async (req, res) => {
		...
		if (type === "csv") {
			...
			const csv = buildCsv(items, headers);
	...
	router.get("/reports/:reportId/export", authRequired(), asyncHandler(async (req, res) => {
		...
		const csv = buildCsv([report], ["id", "title", "category", "periodLabel", "status", "summary"]);
		res.setHeader("Content-Type", "text/csv; charset=utf-8");
